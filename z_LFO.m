...
if checkbox
    [submit,LFO] = z_LFO_GUI(par,handles.par_limites(par));
    if submit
        switch par
            case 1
                set(handles.par_1_LFO,'Value');
                set(handles.par_1_value,'String',strcat('[',num2str(handles.par_limites(par).Min),',',num2str(handles.par_limites(par).Max),'] (f = ',num2str(LFO.frecuencia),'Hz)'));
            case 2
